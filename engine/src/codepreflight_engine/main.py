...
def _automation_status(request: EngineRequest, root: Path, manager: AutomationManager) -> None:
...
def _automation_grant(request: EngineRequest, root: Path, manager: AutomationManager) -> None:
...
def _automation_revoke_grant(
    request: EngineRequest, root: Path, manager: AutomationManager
) -> None:
    complete(
        request.requestId,
...
def _automation_event(request: EngineRequest, root: Path, manager: AutomationManager) -> None:
...
def _automation_worker(request: EngineRequest, root: Path, manager: AutomationManager) -> None:
    ...
    try:
        ...
        if not grant["valid"]:
            waiting = manager.update_job(
                job_id,
                "waiting_for_consent",
                message=("Automation grant is missing or no longer matches provider settings"),
            )
...
def _handle_automation(request: EngineRequest, path: Path) -> None:
    ...
    handlers = {
        ...
        "configure": _automation_configure,
        "grant": _automation_grant,
        "revoke_grant": _automation_revoke_grant,
        "event": _automation_event,
        "worker": _automation_worker,
    }
    handler = handlers.get(action)
    if handler is None:
        raise CodePreflightError(
            "invalid_automation_action", f"Unknown automation action: {action}"
        )
    handler(request, root, manager)


def _handle_workspace(request: EngineRequest, path: Path) -> None:
    complete(request.requestId, RepositoryWorkspace().run(path, request.payload))
    return


def _handle_scan(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
...
def _handle_git_action(request: EngineRequest, path: Path) -> None:
    complete(request.requestId, GitActions().run(path, request.payload))
    return


def _handle_doctor(request: EngineRequest, path: Path) -> None:
    complete(request.requestId, doctor_report(path))
    return


def _handle_cache(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
...
def _handle_provider(request: EngineRequest, path: Path) -> None:
    ...
    if action == "test":
        ...
        if not provider_id:
            raise CodePreflightError("provider_required", "A provider is required for a smoke test")
...
def _handle_commit(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
    root = Path(snapshot.root)
    ...
    if action == "execute":
        ...
        return
    raise CodePreflightError("invalid_commit_action", "Commit action must be prepare or execute")


def _handle_hooks(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
    action = str(request.payload.get("action", "status"))
...
def _handle_explain(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
...
def _handle_review(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
    review_result = ReviewOrchestrator().review(
        Path(snapshot.root),
        request.payload,
...
def _handle_panel_review(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
...
def _handle_pr_prepare(request: EngineRequest, path: Path) -> None:
    snapshot = RepositoryInspector().inspect(path)
...
COMMAND_HANDLERS = {
    ...
    "pr_prepare": _handle_pr_prepare,
    "status": _handle_status,
    "providers": _handle_provider,
}
...
def dispatch(request: EngineRequest) -> None:
    ...
    handler(request, Path(request.repositoryPath).resolve())
